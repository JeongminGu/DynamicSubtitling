...
namespace SDKTemplate
{
    ...
    public sealed partial class Scenario1_ExampleOperations : Page
    {
        ...
        //private const int IMAGE_ROWS = 240;
        //private const int IMAGE_COLS = 320;

        public OpenCVHelper _helper;
        public static OpenCVHelper _Recog;
        private DispatcherTimer _FPSTimer = null;

        public BackgroundWorker worker;
        public static string translated_word = "";

        public static int k = 0;
        unsafe


        public static void voice_recognizer()
        {
            while (true)
            {
                Thread thread2 = new Thread(new ThreadStart(Scenario1_ExampleOperations.voice_recognizer_set));
                thread2.Start();
                thread2.Join();
            }
        }

        public static void voice_recognizer_set()
        {
        ...
        {
            var config = SpeechConfig.FromSubscription("85b5b8b5c10e48588f3df8b2f77d39ac", "koreacentral");
            ...
            using (var recognizer = new SpeechRecognizer(config))
            {
                ...
                else if (result.Reason == ResultReason.Canceled)
                {
                    ...
                    {
                        translated_word = cancellation.ErrorDetails;
                    }
                }
            }
        }
        ...
        protected override async void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            currentOperation = OperationType.Main;
        ...
        private void ColorFrameReader_FrameArrivedAsync(MediaFrameReader sender, MediaFrameArrivedEventArgs args)
        {
            ...
            if (frame != null)
            {
                ...
                if (inputBitmap != null)
                {
                    ...
                    if (currentOperation == OperationType.Main)
                    {
                        _helper.Main(originalBitmap, outputBitmap, translated_word);
                    }
        ...
        private void OperationComboBox_SelectionChanged(object sender, RoutedEventArgs e)
        {
            ...
            if (OperationType.Main == currentOperation)
            {
                this.CurrentOperationTextBlock.Text = "Current: Main";
            }
            else
            {
